...
void loop() {
  ...
  {
    mag.readMag(&x, &y, &z);
    magnitude = sqrt(pow(x*mag.x_scale, 2) + pow(y*mag.y_scale, 2)); //x_scale and y_scale are set by calibration! Do not modify them!
    if (count != 0){
          if (abs(oldMag-magnitude)/oldMag > 0.05){
